...
import { Plus, Loader2, Zap, Heart, Thermometer, Activity, Shield } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";

const workoutLog = [
  { date: "2025-01-10", type: "HIIT", duration: "28 min", hr: 162, cal: 487, confidence: "94%", rating: "Excellent" },
  { date: "2025-01-09", type: "Running", duration: "35 min", hr: 155, cal: 410, confidence: "91%", rating: "Good" },
  { date: "2025-01-08", type: "Yoga", duration: "45 min", hr: 98, cal: 185, confidence: "88%", rating: "Good" },
  { date: "2025-01-07", type: "Weightlifting", duration: "50 min", hr: 138, cal: 385, confidence: "92%", rating: "Excellent" },
  { date: "2025-01-06", type: "Cycling", duration: "40 min", hr: 148, cal: 320, confidence: "89%", rating: "Moderate" },
];
          ...
          {result && (
            <div className="space-y-4 rounded-lg border border-nexus-cyan/30 bg-nexus-cyan/5 p-6">
              ...
              <p className="text-xs text-nexus-text-secondary text-right">
                Model: {result.prediction.model_version} • {result.prediction.features_used} features
              </p>
            </div>
          )}
        </MotionCard>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {workoutLog.map((w, i) => (
              ...
              >
                <td className="px-6 py-4 font-mono text-nexus-text-secondary">{w.date}</td>
                <td className="px-6 py-4 text-nexus-text-primary">{w.type}</td>
                <td className="px-6 py-4 text-nexus-text-secondary">{w.duration}</td>
                <td className="px-6 py-4 font-mono text-nexus-text-primary">{w.hr} bpm</td>
                <td className="px-6 py-4 font-mono text-nexus-cyan">{w.cal} kcal</td>
                <td className="px-6 py-4 font-mono text-nexus-text-secondary">{w.confidence}</td>
                <td className="px-6 py-4">
                  <span
                    className={`inline-flex rounded-full border px-2.5 py-0.5 text-xs font-semibold ${ratingColor[w.rating]}`}
                  >
                    {w.rating}
                  </span>
                </td>
